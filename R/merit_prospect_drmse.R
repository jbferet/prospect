...
merit_prospect_drmse <- function(x, spec_prospect, refl, tran, input_prospect,
                                 parms_to_estimate){
  x[x < 0] <- 0
  ...
  fcr <- fct <- 0
  if (!is.null(refl))
    fcr <- sqrt(sum((diff(refl) - diff(RT$reflectance))**2) / (length(RT$reflectance)-1))
  if (!is.null(tran))
    fct <- sqrt(sum((diff(tran) - diff(RT$transmittance))**2) / (length(RT$transmittance)-1))
  fc <- fcr + fct
  return(fc)
}
