...
`chl`, `car`, `ant`, `ewt`, and `lma` are assessed when setting `parms_to_estimate  <- 'ALL'`
...
`chl`, `car`, `ant`, `ewt`, and `lma` can be assessed. However, no optimal spectral domain has been investigated for ant so far, so the VNIR domain from 400 nm to 800 nm is used in this case...
