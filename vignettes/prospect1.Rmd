...
* the refractive index is the same for all leaves. It should theoretically change with leaf properties
...
The current version of the model implemented in package `prospect` is __PROSPECT-PRO__, which includes the following biochemical constituents (defined as _C<sub>i</sub>_ in Fig. 1):
...
In the forward mode, PROSPECT simulates leaf optical properties, i.e. reflectance and transmittance, based on a set of input parameters corresponding to the biochemical constituents and the N structure parameter.
...
In the inverse mode, an algorithm is used to estimate the biochemical constituents from the leaf optical properties.
