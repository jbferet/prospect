...
BiochTruth <- c(DataBioch_VAL[,6]-DataBioch_VAL[,12])[[1]]
...
number_features <- 10
ListRes_CBC <- optimal_features_SFS(Refl = Refl_VAL, Tran = Tran_VAL, lambda = lambda[[1]], BiochTruth = BiochTruth, Target = Target,
...
#  possibility in case user wants to perform feature selection in several steps
number_features <- 50
ListRes_CBC <- optimal_features_SFS(Refl = Refl_VAL, Tran = Tran_VAL, lambda = lambda[[1]], BiochTruth = BiochTruth, Target = Target,
                                    Parms2Estimate = Parms2Estimate, InitValues = InitValues, SpecPROSPECT = SpecPROSPECT,
                                    spectral_domain = spectral_domain, spectral_width = spectral_width, number_features = number_features,
                                    PROSPECT_version = 'PRO',nbCPU = nbCPU, Continue = TRUE, AlreadyDone = ListRes_CBC)

#  save list containing all information about SFS
# - optimal spectral features for each step
# - corresponding RMSE
# corresponding estimated parameter of interest for each additional feature
save(list = 'ListRes_CBC',file = 'Optimal_CBC_VAL.RData')
